...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Allow CrucibAI / custom app domains (add your production domain as needed)
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*crucibai\.(dev|io|com)$/)) {
        return true;
      }
      ...
      try {
        ...
        for (const change of variableEditChanges) {
          ...
          if (result.success) {
            ...
            try {
              execSync(`git -c user.name="visual-edit" -c user.email="support@crucibai.dev" add "${result.file}"`);
              execSync(`git -c user.name="visual-edit" -c user.email="support@crucibai.dev" commit -m "visual_edit_variable_${timestamp}"`);
            } catch (gitError) {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="support@crucibai.dev" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="support@crucibai.dev" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
