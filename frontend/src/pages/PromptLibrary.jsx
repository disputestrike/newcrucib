      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="space-y-4">
          {tab === 'templates' && templates.map((t) => (
            <div key={t.id} className="p-4 rounded-xl border border-zinc-800 bg-zinc-900/50">
              ...
              <p className="text-sm text-zinc-400 line-clamp-2">{t.prompt}</p>
            </div>
          ))}
          {tab === 'saved' && (saved.length === 0 ? <p className="text-zinc-500">No saved prompts yet.</p> : saved.map((p) => (
          ...
          {tab === 'recent' && (recent.length === 0 ? <p className="text-zinc-500">No recent prompts.</p> : recent.map((p, i) => (
